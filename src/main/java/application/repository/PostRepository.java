...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = "SELECT * FROM posts WHERE is_active = ?1 AND moderation_status = ?2 " +
            "AND time <= ?3 ORDER BY time ASC", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM posts WHERE is_active = ?1 AND moderation_status = ?2 " +
            "AND time <= ?3 ORDER BY time DESC", nativeQuery = true)
    List<Post> findAllByIsActiveAndModerationStatusAndTimeLessThanEqualAndOrderByTimeDes
            (boolean isActive, String moderationStatus, Timestamp time);

    @Query(value = "SELECT *, (SELECT COUNT(*) AS comments_number FROM post_comments WHERE " +
            "post_comments.post_id = posts.id) FROM posts WHERE is_active = ?1 AND " +
            "moderation_status = ?2 AND time <= ?3 ORDER BY comments_number DESC", nativeQuery = true)
    List<Post> findAllByIsActiveAndModerationStatusAndTimeLessThanEqualAndOrderByCommentsNumberDes
            (boolean isActive, String moderationStatus, Timestamp time);

    @Query(value = "SELECT *, (SELECT COUNT(*) AS likes_number FROM post_votes WHERE " +
            "post_votes.post_id = posts.id) FROM posts WHERE is_active = ?1 AND " +
            "moderation_status = ?2 AND time <= ?3 ORDER BY likes_number DESC", nativeQuery = true)
    List<Post> findAllByIsActiveAndModerationStatusAndTimeLessThanEqualAndOrderByLikesNumberDes
            (boolean isActive, String moderationStatus, Timestamp time);

    @Query(value = "SELECT * FROM posts WHERE is_active = ?1 AND moderation_status = ?2 " +
            "AND time <= ?3 AND title ILIKE ?4 ORDER BY time DESC", nativeQuery = true)
    ...
    @Query(value = "SELECT p.* FROM posts As p, tag2post AS t2p, tags As t " +
            ...
            "p.moderation_status = ?2 AND p.time <= ?3 AND t.name = ?4 ORDER BY p.time DESC", nativeQuery = true)
    List<Post> findAllPostsByTag
            (boolean isActive, String moderationStatus, Timestamp time, String query);

    @Query(value = "SELECT t.name AS name, COUNT(p.*) AS total FROM posts As p, tag2post AS t2p, tags As t " +
    ...
    @Query(value = "SELECT to_char(time, 'YYYY-MM-DD') AS name, COUNT(*)  AS total " +
            "FROM posts GROUP BY name " +
            "ORDER BY name", nativeQuery = true)
    List<IPostCount> countPostsByDay();
